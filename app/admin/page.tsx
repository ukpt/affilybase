...
export default function Admin() {
  ...
  const vendeursFiltres = useMemo(() => {
    ...
    if (filterPlan !== 'all') {
      result = result.filter(v => (v.plan || 'free') === filterPlan)
    }
    ...
    if (filterActivite === 'actif') {
      result = result.filter(v => v.nbCodes > 0)
    } else if (filterActivite === 'inactif') {
      result = result.filter(v => v.nbCodes === 0)
    }

    result.sort((a, b) => {
      if (sortBy === 'revenu') return b.revenuTotal - a.revenuTotal
      if (sortBy === 'codes') return b.nbCodes - a.nbCodes
      if (sortBy === 'ventes') return b.nbVentes - a.nbVentes
      return new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
    })

    return result
  }, [vendeurs, filterPlan, search, sortBy, filterActivite])

  const moisLabel = (n: number) => n === 0 ? '—' : n === 1 ? '1 mois' : `${n} mois`

  const planStyle = (plan: string) => ({
    background: plan === 'starter' ? '#E1F5EE' : plan === 'pro' ? '#EEEDFE' : plan === 'business' ? '#F5F2EC' : '#FAEEDA',
    color: plan === 'starter' ? '#085041' : plan === 'pro' ? '#3C3489' : plan === 'business' ? '#1a1a1a' : '#633806',
  })
    ...
    <div style={{ minHeight: '100vh', background: '#F5F2EC', padding: '2rem' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '8px', marginBottom: '1rem' }}>
          {[
            { key: 'all', label: 'Tous', value: stats.total, bg: '#F5F2EC', color: '#1a1a1a' },
            { key: 'free', label: 'Free · 0 €', value: stats.free, bg: '#FAEEDA', color: '#633806' },
            { key: 'starter', label: 'Starter · 4,99 €', value: stats.starter, bg: '#E1F5EE', color: '#085041' },
            { key: 'pro', label: 'Pro · 9,99 €', value: stats.pro, bg: '#EEEDFE', color: '#3C3489' },
            { key: 'business', label: 'Business · 39,99 €', value: stats.business, bg: '#F5F2EC', color: '#1a1a1a' },
          ].map(({ key, label, value, bg, color }) => (
            <div key={key} onClick={() => setFilterPlan(key)} style={{ background: bg, border: `2px solid ${filterPlan === key ? color : 'transparent'}`, borderRadius: '8px', padding: '0.75rem', textAlign: 'center', cursor: 'pointer' }}>
              <div style={{ fontSize: '22px', fontWeight: 500, color }}>{value}</div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '1rem', flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: '200px', padding: '8px 12px', border: '0.5px solid #ddd8ce', borderRadius: '6px', fontSize: '13px', background: '#fff', outline: 'none' }}
          />
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: '8px 12px', border: '0.5px solid #ddd8ce', borderRadius: '6px', fontSize: '13px', background: '#fff', cursor: 'pointer' }}>
            <option value="date">Trier par date</option>
            <option value="revenu">Trier par revenus</option>
            <option value="codes">Trier par codes créés</option>
            <option value="ventes">Trier par ventes</option>
          </select>
          <select value={filterActivite} onChange={e => setFilterActivite(e.target.value)} style={{ padding: '8px 12px', border: '0.5px solid #ddd8ce', borderRadius: '6px', fontSize: '13px', background: '#fff', cursor: 'pointer' }}>
            <option value="all">Toute activité</option>
            <option value="actif">Actifs (codes créés)</option>
        ...
        <div style={{ background: '#fff', border: '0.5px solid #ddd8ce', borderRadius: '10px', padding: '1.25rem' }}>
          <div style={{ fontSize: '13px', fontWeight: 500, marginBottom: '1rem', paddingBottom: '8px', borderBottom: '0.5px solid #ddd8ce', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: '12px', color: '#888' }}>{vendeursFiltres.length} affiché{vendeursFiltres.length > 1 ? 's' : ''}</span>
          </div>

          {vendeursFiltres.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: '#888', fontSize: '13px' }}>Aucun vendeur trouvé</div>
          ) : (
                ...
                <div key={i} style={{ border: '0.5px solid #ddd8ce', borderRadius: '8px', padding: '1rem', background: '#F5F2EC' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px', flexWrap: 'wrap', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ ...planStyle(v.plan || 'free'), fontSize: '11px', padding: '3px 10px', borderRadius: '20px', fontWeight: 500 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px', paddingTop: '10px', borderTop: '0.5px solid #ddd8ce' }}>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '14px', fontWeight: 500, color: v.revenuTotal > 0 ? '#1D9E75' : '#888' }}>{v.revenuTotal.toFixed(2)} €</div>
